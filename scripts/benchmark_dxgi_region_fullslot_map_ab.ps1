param(
    [string]$BaselinePath = "target/perf/dxgi-region-fullslot-map-ab-baseline.csv",
    [string]$TargetLabel = "dxgi-region-animated-fullslot-map",
    [int]$WarmupFrames = 80,
...
$workloadInfoPath = Join-Path $repoRoot "target/perf/dxgi-region-fullslot-map-workload.json"
...
function Invoke-DxgiRegionBenchmark {
    param(
        ...
        [bool]$EnableFullSlotMapFastpath
    )

    $oldFullSlotMap = $env:SNOW_CAPTURE_DXGI_ENABLE_REGION_FULL_SLOT_MAP_FASTPATH

    try {
        if ($EnableFullSlotMapFastpath) {
            $env:SNOW_CAPTURE_DXGI_ENABLE_REGION_FULL_SLOT_MAP_FASTPATH = "1"
        } else {
            Remove-Item Env:SNOW_CAPTURE_DXGI_ENABLE_REGION_FULL_SLOT_MAP_FASTPATH -ErrorAction SilentlyContinue
        }
        ...
        if ($null -ne $oldFullSlotMap) {
            $env:SNOW_CAPTURE_DXGI_ENABLE_REGION_FULL_SLOT_MAP_FASTPATH = $oldFullSlotMap
        } else {
            Remove-Item Env:SNOW_CAPTURE_DXGI_ENABLE_REGION_FULL_SLOT_MAP_FASTPATH -ErrorAction SilentlyContinue
        }
    }
...
try {
    ...
    Write-Host "Collecting baseline (fastpath disabled)..."
    $baselineExitCode = Invoke-DxgiRegionBenchmark -BenchmarkArgs ($commonArgs + @("--save-baseline", $resolvedBaselinePath)) -EnableFullSlotMapFastpath $false
    if ($baselineExitCode -ne 0) {
        exit $baselineExitCode
    }

    if ($SaveBaselineOnly) {
        Write-Host "Saved disabled-fastpath baseline to $resolvedBaselinePath"
        exit 0
    }
    ...
    ) -EnableFullSlotMapFastpath $true
