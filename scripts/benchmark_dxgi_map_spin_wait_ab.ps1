param(
    [string]$BaselinePath = "target/perf/dxgi-map-spin-wait-ab-baseline.csv",
    [string]$TargetLabel = "dxgi-region-animated-map-spin",
    [int]$WarmupFrames = 80,
...
$workloadInfoPath = Join-Path $repoRoot "target/perf/dxgi-map-spin-workload.json"
...
function Invoke-DxgiRegionBenchmark {
    param(
        ...
        [bool]$DisableMapSpinWait
    )

    $oldDisableMapSpinWait = $env:SNOW_CAPTURE_DISABLE_D3D11_MAP_SPIN_WAIT

    try {
        if ($DisableMapSpinWait) {
            $env:SNOW_CAPTURE_DISABLE_D3D11_MAP_SPIN_WAIT = "1"
        } else {
            Remove-Item Env:SNOW_CAPTURE_DISABLE_D3D11_MAP_SPIN_WAIT -ErrorAction SilentlyContinue
        }
        ...
        if ($null -ne $oldDisableMapSpinWait) {
            $env:SNOW_CAPTURE_DISABLE_D3D11_MAP_SPIN_WAIT = $oldDisableMapSpinWait
        } else {
            Remove-Item Env:SNOW_CAPTURE_DISABLE_D3D11_MAP_SPIN_WAIT -ErrorAction SilentlyContinue
        }
    }
...
try {
    ...
    Write-Host "Collecting baseline with D3D11 map spin wait disabled..."
    $baselineExitCode = Invoke-DxgiRegionBenchmark -BenchmarkArgs ($commonArgs + @("--save-baseline", $resolvedBaselinePath)) -DisableMapSpinWait $true
    if ($baselineExitCode -ne 0) {
        exit $baselineExitCode
    }

    if ($SaveBaselineOnly) {
        Write-Host "Saved disabled-map-spin baseline to $resolvedBaselinePath"
        exit 0
    }
