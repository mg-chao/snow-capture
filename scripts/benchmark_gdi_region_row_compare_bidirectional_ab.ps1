param(
    [string]$BaselinePath = "target/perf/gdi-region-row-compare-bidirectional-ab-baseline.csv",
    [string]$TargetLabel = "gdi-region-row-compare-bidirectional",
    [int]$WarmupFrames = 80,
...
$workloadInfoPath = Join-Path $repoRoot "target/perf/gdi-region-row-compare-bidirectional-workload.json"
...
function Invoke-GdiRegionBenchmark {
    param(
        ...
        [bool]$DisableBidirectionalCompare
    )

    $oldDisableBidirectional = $env:SNOW_CAPTURE_DISABLE_GDI_ROW_COMPARE_BIDIRECTIONAL

    try {
        if ($DisableBidirectionalCompare) {
            $env:SNOW_CAPTURE_DISABLE_GDI_ROW_COMPARE_BIDIRECTIONAL = "1"
        } else {
            Remove-Item Env:SNOW_CAPTURE_DISABLE_GDI_ROW_COMPARE_BIDIRECTIONAL -ErrorAction SilentlyContinue
        }
        ...
        if ($null -ne $oldDisableBidirectional) {
            $env:SNOW_CAPTURE_DISABLE_GDI_ROW_COMPARE_BIDIRECTIONAL = $oldDisableBidirectional
        } else {
            Remove-Item Env:SNOW_CAPTURE_DISABLE_GDI_ROW_COMPARE_BIDIRECTIONAL -ErrorAction SilentlyContinue
        }
    }
...
try {
    ...
    Write-Host "Collecting baseline with bidirectional row compare disabled..."
    $baselineExitCode = Invoke-GdiRegionBenchmark -BenchmarkArgs ($commonArgs + @("--save-baseline", $resolvedBaselinePath)) -DisableBidirectionalCompare $true
    ...
    ) -DisableBidirectionalCompare $false
