...
Tip: compare DXGI region move-rect reconstruction in window mode:
  optimized: cargo run --release --example benchmark -- --backends dxgi --window-under-cursor
  legacy (PowerShell): $env:SNOW_CAPTURE_DXGI_DISABLE_REGION_MOVE_RECONSTRUCT=1; cargo run --release --example benchmark -- --backends dxgi --window-under-cursor; Remove-Item Env:SNOW_CAPTURE_DXGI_DISABLE_REGION_MOVE_RECONSTRUCT
  legacy (cmd.exe):    setlocal && set SNOW_CAPTURE_DXGI_DISABLE_REGION_MOVE_RECONSTRUCT=1 && cargo run --release --example benchmark -- --backends dxgi --window-under-cursor && endlocal
  legacy (bash):       SNOW_CAPTURE_DXGI_DISABLE_REGION_MOVE_RECONSTRUCT=1 cargo run --release --example benchmark -- --backends dxgi --window-under-cursor
...
Tip: compare WGC dense dirty-region fallback in region mode:
  ...
  legacy (PowerShell): $env:SNOW_CAPTURE_WGC_DISABLE_REGION_DIRTY_DENSE_FALLBACK=1; cargo run --release --example benchmark -- --backends wgc --region-center 1920x1080 --sample-interval-ms 8; Remove-Item Env:SNOW_CAPTURE_WGC_DISABLE_REGION_DIRTY_DENSE_FALLBACK
...
Tip: compare the GDI adaptive parallel span mode-history heuristic:
  optimized: cargo run --release --example benchmark -- --backends gdi --region-center 2560x1440
  legacy (PowerShell): $env:SNOW_CAPTURE_DISABLE_GDI_PARALLEL_SPAN_MODE_HISTORY=1; cargo run --release --example benchmark -- --backends gdi --region-center 2560x1440; Remove-Item Env:SNOW_CAPTURE_DISABLE_GDI_PARALLEL_SPAN_MODE_HISTORY
  legacy (cmd.exe):    setlocal && set SNOW_CAPTURE_DISABLE_GDI_PARALLEL_SPAN_MODE_HISTORY=1 && cargo run --release --example benchmark -- --backends gdi --region-center 2560x1440 && endlocal
  legacy (bash):       SNOW_CAPTURE_DISABLE_GDI_PARALLEL_SPAN_MODE_HISTORY=1 cargo run --release --example benchmark -- --backends gdi --region-center 2560x1440"
                );
                std::process::exit(0);
            }
            other => {
