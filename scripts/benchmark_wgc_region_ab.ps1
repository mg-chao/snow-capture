param(
    [string]$BaselinePath = "target/perf/wgc-region-ab-baseline.csv",
    [string]$TargetLabel = "wgc-region-animated",
    [int]$WarmupFrames = 80,
    [int]$MeasureFrames = 700,
    [int]$Rounds = 6,
    [double]$SampleIntervalMs = 10.0,
...
$workloadInfoPath = "target/perf/wgc-region-workload.json"
...
function Invoke-WgcBenchmark {
    param(
        [string[]]$BenchmarkArgs,
        [bool]$DisableOptimizations
    )

    $oldDisableRegionCopy = $env:SNOW_CAPTURE_WGC_DISABLE_REGION_DIRTY_GPU_COPY
    $oldDisableDuplicateFastpath = $env:SNOW_CAPTURE_WGC_DISABLE_DUPLICATE_DIRTY_FASTPATH

    try {
        if ($DisableOptimizations) {
            $env:SNOW_CAPTURE_WGC_DISABLE_REGION_DIRTY_GPU_COPY = "1"
            $env:SNOW_CAPTURE_WGC_DISABLE_DUPLICATE_DIRTY_FASTPATH = "1"
        } else {
            Remove-Item Env:SNOW_CAPTURE_WGC_DISABLE_REGION_DIRTY_GPU_COPY -ErrorAction SilentlyContinue
            Remove-Item Env:SNOW_CAPTURE_WGC_DISABLE_DUPLICATE_DIRTY_FASTPATH -ErrorAction SilentlyContinue
        }
        ...
        if ($null -ne $oldDisableRegionCopy) {
            $env:SNOW_CAPTURE_WGC_DISABLE_REGION_DIRTY_GPU_COPY = $oldDisableRegionCopy
        } else {
            Remove-Item Env:SNOW_CAPTURE_WGC_DISABLE_REGION_DIRTY_GPU_COPY -ErrorAction SilentlyContinue
        }

        if ($null -ne $oldDisableDuplicateFastpath) {
            $env:SNOW_CAPTURE_WGC_DISABLE_DUPLICATE_DIRTY_FASTPATH = $oldDisableDuplicateFastpath
        } else {
            Remove-Item Env:SNOW_CAPTURE_WGC_DISABLE_DUPLICATE_DIRTY_FASTPATH -ErrorAction SilentlyContinue
        }
    }
...
try {
    ...
    Write-Host "Collecting baseline with optimizations disabled..."
    $baselineExitCode = Invoke-WgcBenchmark -BenchmarkArgs ($commonArgs + @("--save-baseline", $BaselinePath)) -DisableOptimizations $true
    ...
    $optimizedExitCode = Invoke-WgcBenchmark -BenchmarkArgs (
        $commonArgs + @(
