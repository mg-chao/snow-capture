param(
    [string]$BaselinePath = "target/perf/gdi-window-too-dirty-probe-ab-baseline.csv",
    [string]$TargetLabel = "gdi-window-too-dirty-probe",
    [int]$WarmupFrames = 80,
    [int]$MeasureFrames = 700,
    [int]$Rounds = 5,
    [double]$SampleIntervalMs = 8.0,
    [double]$MaxRegressionPct = 5.0,
    [double]$MaxDuplicatePct = 50.0,
    [int]$WorkloadWidth = 1920,
    [int]$WorkloadHeight = 1080,
    [int]$WorkloadBoxSize = 4096,
    [int]$WorkloadX = 120,
    [int]$WorkloadY = 80,
...
$workloadInfoPath = Join-Path $repoRoot "target/perf/gdi-window-too-dirty-probe-workload.json"
...
function Invoke-GdiBenchmark {
    param(
        ...
        [bool]$DisableTooDirtyProbe
    )

    $oldDisableTooDirtyProbe = $env:SNOW_CAPTURE_DISABLE_GDI_TOO_DIRTY_PROBE

    try {
        if ($DisableTooDirtyProbe) {
            $env:SNOW_CAPTURE_DISABLE_GDI_TOO_DIRTY_PROBE = "1"
        } else {
            Remove-Item Env:SNOW_CAPTURE_DISABLE_GDI_TOO_DIRTY_PROBE -ErrorAction SilentlyContinue
        }
        ...
        if ($null -ne $oldDisableTooDirtyProbe) {
            $env:SNOW_CAPTURE_DISABLE_GDI_TOO_DIRTY_PROBE = $oldDisableTooDirtyProbe
        } else {
            Remove-Item Env:SNOW_CAPTURE_DISABLE_GDI_TOO_DIRTY_PROBE -ErrorAction SilentlyContinue
        }
    }
...
try {
    ...
    $effectiveTargetLabel = "{0}-{1}x{2}-fullredraw" -f $TargetLabel, $workloadInfo.width, $workloadInfo.height
    ...
    Write-Host "Collecting baseline with too-dirty probe disabled..."
    $baselineExitCode = Invoke-GdiBenchmark -BenchmarkArgs ($commonArgs + @("--save-baseline", $resolvedBaselinePath)) -DisableTooDirtyProbe $true
