param(
    [string]$BaselinePath = "target/perf/dxgi-source-resource-cast-ab-baseline.csv",
    [string]$TargetLabel = "dxgi-source-resource-cast",
    [int]$WarmupFrames = 80,
...
$workloadInfoPath = "target/perf/dxgi-source-resource-cast-workload.json"
...
function Invoke-DxgiSourceResourceBenchmark {
    param(
        [string[]]$BenchmarkArgs,
        [bool]$DisableOptimizations
    )

    $oldDisableBorrowedSourceResource = $env:SNOW_CAPTURE_DXGI_DISABLE_BORROWED_SOURCE_RESOURCE
    $oldDisableDirtyRectShortcut = $env:SNOW_CAPTURE_DISABLE_DIRTY_RECT_NON_OVERLAP_SHORTCUT

    try {
        if ($DisableOptimizations) {
            $env:SNOW_CAPTURE_DXGI_DISABLE_BORROWED_SOURCE_RESOURCE = "1"
            $env:SNOW_CAPTURE_DISABLE_DIRTY_RECT_NON_OVERLAP_SHORTCUT = "1"
        } else {
            Remove-Item Env:SNOW_CAPTURE_DXGI_DISABLE_BORROWED_SOURCE_RESOURCE -ErrorAction SilentlyContinue
            Remove-Item Env:SNOW_CAPTURE_DISABLE_DIRTY_RECT_NON_OVERLAP_SHORTCUT -ErrorAction SilentlyContinue
        }
        ...
        if ($null -ne $oldDisableBorrowedSourceResource) {
            $env:SNOW_CAPTURE_DXGI_DISABLE_BORROWED_SOURCE_RESOURCE = $oldDisableBorrowedSourceResource
        } else {
            Remove-Item Env:SNOW_CAPTURE_DXGI_DISABLE_BORROWED_SOURCE_RESOURCE -ErrorAction SilentlyContinue
        }

        if ($null -ne $oldDisableDirtyRectShortcut) {
            $env:SNOW_CAPTURE_DISABLE_DIRTY_RECT_NON_OVERLAP_SHORTCUT = $oldDisableDirtyRectShortcut
        } else {
            Remove-Item Env:SNOW_CAPTURE_DISABLE_DIRTY_RECT_NON_OVERLAP_SHORTCUT -ErrorAction SilentlyContinue
        }
    }
...
try {
    ...
    Write-Host "Collecting baseline with source-resource cast/dirty-shortcut optimizations disabled..."
    $baselineExitCode = Invoke-DxgiSourceResourceBenchmark -BenchmarkArgs ($commonArgs + @("--save-baseline", $BaselinePath)) -DisableOptimizations $true
    ...
    $optimizedExitCode = Invoke-DxgiSourceResourceBenchmark -BenchmarkArgs (
        $commonArgs + @(
