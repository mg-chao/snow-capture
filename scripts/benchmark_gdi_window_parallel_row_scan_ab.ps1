param(
    [string]$BaselinePath = "target/perf/gdi-window-parallel-row-scan-ab-baseline.csv",
    [string]$TargetLabel = "gdi-window-parallel-row-scan",
    [int]$WarmupFrames = 80,
...
$workloadInfoPath = Join-Path $repoRoot "target/perf/gdi-window-parallel-row-scan-workload.json"
...
function Invoke-GdiBenchmark {
    param(
        ...
        [bool]$DisableParallelRowScan
    )

    $oldDisableParallel = $env:SNOW_CAPTURE_DISABLE_GDI_PARALLEL_ROW_SCAN

    try {
        if ($DisableParallelRowScan) {
            $env:SNOW_CAPTURE_DISABLE_GDI_PARALLEL_ROW_SCAN = "1"
        } else {
            Remove-Item Env:SNOW_CAPTURE_DISABLE_GDI_PARALLEL_ROW_SCAN -ErrorAction SilentlyContinue
        }
        ...
        if ($null -ne $oldDisableParallel) {
            $env:SNOW_CAPTURE_DISABLE_GDI_PARALLEL_ROW_SCAN = $oldDisableParallel
        } else {
            Remove-Item Env:SNOW_CAPTURE_DISABLE_GDI_PARALLEL_ROW_SCAN -ErrorAction SilentlyContinue
        }
    }
...
try {
    ...
    Write-Host "Collecting baseline with parallel row scan disabled..."
    $baselineExitCode = Invoke-GdiBenchmark -BenchmarkArgs ($commonArgs + @("--save-baseline", $resolvedBaselinePath)) -DisableParallelRowScan $true
